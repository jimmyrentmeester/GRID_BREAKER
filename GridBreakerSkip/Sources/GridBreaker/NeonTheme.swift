...
struct TrailSkin: Identifiable, Sendable, Equatable {
    ...
    func dotPath(at p: CGPoint, size: CGFloat) -> Path {
        ...
        case .square: return Path(roundedRect: r, cornerRadius: max(1.0, size * 0.2))
        case .diamond:
            var d = Path()
...
enum TrailSkins {
    ...
    /// nonisolated(unsafe): single-writer global set on the main thread (Swift 6 / Skip).
    nonisolated(unsafe) static var equipped: TrailSkin = none
}
...
enum NeonTheme {
    ...
    /// nonisolated(unsafe): single-writer global set on the main thread (Swift 6 / Skip).
    nonisolated(unsafe) static var current: Palette = Palettes.classic
