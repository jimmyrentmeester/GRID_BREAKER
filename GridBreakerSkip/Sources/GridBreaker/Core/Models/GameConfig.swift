...
/// A simple inclusive [lo, hi] integer bound — used instead of `ClosedRange<Int>`,
/// which Skip maps to a Kotlin `IntRange` lacking `.lowerBound`/`.upperBound`.
...
struct GameConfig: Sendable {
    ...
    static func campaign(for core: DataCore) -> GameConfig {
        ...
        // Explicit Double `: 0.0` else-branches: a `? 0.20 : 0` ternary mixes Double/Int
        // → Skip types it as a boxed Number that won't assign to a Double (#29/#42).
        c.armoredSpawnChance = core.armored ? 0.20 : 0.0
        c.firewallSpawnChance = core.bombs ? 0.15 : 0.0
        c.feverEnabled = core.fever
        c.cacheSpawnChance = core.cache ? 0.05 : 0.0
        c.wormSpawnChance = core.worm ? 0.08 : 0.0
        ...
        let halfway = core.targetScore / 2
        c.gridEscalationScore = core.grid4x4 ? (halfway < 1 ? 1 : halfway) : nil
        return c
    }
    ...
    // NOTE: the methods below use explicit ternary clamps instead of `min`/`max`.
    // Skip's generic `min`/`max` widen to a boxed `Number & Comparable` that Kotlin
    // won't unbox back to Double/Int (Skip pitfall #42), so every clamp is spelled out.

    /// Credits awarded for a final score (deterministic, paid once on game over).
    func credits(forScore score: Int) -> Int {
        let c = Int(Double(score) * creditsPerScore)
        return c < 0 ? 0 : c
    }
    ...
    func nodeLifespan(atScore score: Int) -> TimeInterval {
        ...
        return scaled < minNodeLifespan ? minNodeLifespan : scaled
    }
    ...
    func spawnInterval(atScore score: Int) -> TimeInterval {
        ...
        return scaled < minSpawnInterval ? minSpawnInterval : scaled
    }
    ...
    func objectiveGap(atScore score: Int) -> TimeInterval {
        ...
        return scaled < minObjectiveInterval ? minObjectiveInterval : scaled
    }
    ...
    /// Returns a `SizeBounds` (lo/hi) — Skip maps Swift `ClosedRange<Int>` to a Kotlin
    /// `IntRange` that exposes no `.lowerBound`/`.upperBound` (#55), so we don't use it.
    func daemonSetSizeRange(atScore score: Int) -> SizeBounds {
        let step = daemonSetSizeRampScore > 0 ? score / daemonSetSizeRampScore : daemonSetMaxSize - daemonSetMinSize
        var rampedMax = daemonSetMinSize + step
        if rampedMax > daemonSetMaxSize { rampedMax = daemonSetMaxSize }
        if rampedMax < daemonSetMinSize { rampedMax = daemonSetMinSize }
        return SizeBounds(lo: daemonSetMinSize, hi: rampedMax)
    }

    /// Effective DMZ zone size range at the given score (same stepping pattern as sets).
    func dmzSizeRange(atScore score: Int) -> SizeBounds {
        let step = dmzSizeRampScore > 0 ? score / dmzSizeRampScore : dmzMaxSize - dmzMinSize
        var rampedMax = dmzMinSize + step
        if rampedMax > dmzMaxSize { rampedMax = dmzMaxSize }
        if rampedMax < dmzMinSize { rampedMax = dmzMinSize }
        return SizeBounds(lo: dmzMinSize, hi: rampedMax)
    }
    ...
    func dmzOverrunPace(atScore score: Int) -> TimeInterval {
        ...
        return scaled < minDmzOverrunInterval ? minDmzOverrunInterval : scaled
    }
    ...
    func targetActiveNodes(atScore score: Int, gridSize: GridSize) -> Int {
        let ceiling = gridSize.cellCount - 1
