...
## ✅ Cosmetics 2.0 — BUILT (2026-07-01/02, on main; ships as the free update after v1.3)
...
4. ✅ **Shop tabs + Monolith app icon** (built + sim-verified 2026-07-02): the shop now
   has PALETTES / TRAILS / GLYPHS / ICONS tabs (the catalog outgrew one scroll). The
   **Monolith Gold alternate app icon** (earn-only, 16/16 cores) ships as an
   `AppIconMonolith.appiconset` generated by the parameterized `scripts/makeicon.swift`
   (`gold` style; `classic` output stays byte-identical to the shipped icon) +
   `ASSETCATALOG_COMPILER_ALTERNATE_APPICON_NAMES`. Equip state lives in iOS itself
   (`UIApplication.alternateIconName`); ownership (`ownedIconIDs`) rides the prestige
   sweep. Verified end-to-end in-sim with a campaignProgress=16 save fixture: sweep
   grants → ICONS tab shows Owned → EQUIP → iOS confirm alert → gold icon on the home
   screen. Remaining idea (explicitly out of scope): animated Monolith Gold palette.
   No rarity tiers — ever.

**👤 Pending:** an on-device feel pass over the whole Cosmetics 2.0 update (prestige
toast, live preview, glyph readability at speed, icon switch) before it ships.
